<?php
  ...
  $idligne=0;
  //
  // déterminer les clients déjà présent dans une commande
  // connect, prepare + execute
  $le_SQL = "SELECT idclient FROM client WHERE idclient IN (SELECT idclient FROM commande)";
  $connexion = new connect();
  $requete = $connexion->prepare($le_SQL);    
  $requete -> execute();
  $tabClientDansCommande = $requete -> fetchAll();  // tableau associatif
  $tabListeClientDansCommande=[];                   // tableau indexé
  // on boucle pour récupérer chaque element un par un
  foreach($tabClientDansCommande as $tabvalues){
    foreach($tabvalues as $itemValeur){      
      $tabListeClientDansCommande[] = $itemValeur;
    }
  }  
?>
          ...
          <tr>
              ...
              <td>
                ...
                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#clientSupressionModal<?php echo $idligne?>"
                        <?php if(in_array($itemClient["idclient"], $tabListeClientDansCommande)){ echo 'disabled'; } ?>>
